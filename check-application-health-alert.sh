...
function validateservice {
    ...
    if [ $s_status -eq 0 ]; then
        ...
        echo "FAILED $(/bin/date):WARNING SERVICE($service_name) is stopped" | tee -a "$logpath"
        message="FAILED $(/bin/date):WARNING SERVICE($service_name) is stopped"
        sendalert "$message"
...
if [ $pmem_status == "FAILED:NaN" ] || [ $pcpu_status == "FAILED:NaN" ] || [ $process_fd_status == "FAILED:NaN" ]; then
    ...
    if [ $pcpu_status == "FAILED" ] && [ $pmem_status == "FAILED" ] && [ $sysload_status == "FAILED" ]; then
        echo "WARNING:$(/bin/date) || Uptime(m)=$UPTIME|| PCPU($pcpu),PMEM($pmem) and load_avg($sysload) are above threshold"  >> "$logpath"
        getlastupdatedtime
        echo "WARNING:PCPU($pcpu),PMEM($pmem) and load_avg($sysload) are above threshold:$TIMECHECK:$UPTIME"
        COLOR=ff9d5c
    ...
    elif [ $pcpu_status == "FAILED" ] || [ "$pmem_status" == "FAILED" ]; then
        echo "WARNING:$(/bin/date) || Uptime(m)=$UPTIME|| Current CPU(Usage: $pcpu, Threshold: $pcpu_threshold) OR memory(Usage: $pmem, Threshold: $pmem_threshold) are above threshold "   >> "$logpath"
        getlastupdatedtime
        echo "Current CPU(Usage: $pcpu, Threshold: $pcpu_threshold) OR memory(Usage: $pmem, Threshold: $pmem_threshold) are above threshold "
        COLOR=ff9d5c
        message="WARNING: Current CPU(Usage: $pcpu, Threshold: $pcpu_threshold) OR memory(Usage: $pmem, Threshold: $pmem_threshold) are above threshold, Checked at: $TIMECHECK Uptime: $UPTIME"
        #sendalert $message
        # sudo service "${service_name}" restart
        exit 1
        ...
        echo "WARNING:$(/bin/date) || Uptime(m)=$UPTIME|| PCPU($pcpu) and load_avg($sysload) are above threshold"  >> "$logpath"
        getlastupdatedtime
        echo "WARNING:PCPU($pcpu) and load_avg($sysload) are above threshold:$TIMECHECK:$UPTIME"
        COLOR=ff9d5c
        ...
        sudo service "${service_name}" restart
        COLOR=ff0000
        message="RESTARTED: $(/bin/date) || Uptime(m)=$UPTIME|| Service & Port are not accessible ($url). Checked at: $TIMECHECK || Uptime: $UPTIME"
        sendalert "$message"
        exit 1
