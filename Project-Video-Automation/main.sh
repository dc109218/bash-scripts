...
#Main Folder
folderInIn="13-08-22_9"
folderInMain="/Volumes/vyom_Drive/Video"
masterPath="$folderInMain/$folderInIn"

MaterFile="$masterPath/$(basename $masterPath).txt"

#user_1
fileMp4In1="$(sed '1!d' $MaterFile)"
tagsUser1="$(sed '2!d' $MaterFile)"
...
#user_2
fileMp4In2="$(sed '3!d' $MaterFile)"
tagsUser2="$(sed '4!d' $MaterFile)"
...
#user_3
fileMp4In3="$(sed '5!d' $MaterFile)"
tagsUser3="$(sed '6!d' $MaterFile)"
...
#user_4
fileMp4In4="$(sed '7!d' $MaterFile)"
tagsUser4="$(sed '8!d' $MaterFile)"
...
#user_5
fileMp4In5="$(sed '9!d' $MaterFile)"
tagsUser5="$(sed '10!d' $MaterFile)"
...
hostUrl="https://avatar.roleit.com/rest/api/video/store"
...
function fun_1() {
    ...
    if [[ "$HTTPS1" == 200 ]] ; then
        echo "${GRN}ROLEIT-API-CALL-DONE====${NC}$HTTPS1"
    else
        echo "${RED}ROLEIT-API-CALL-ERROR====${NC}$HTTPS1"
        exit 0
    fi;
...
function fun_2() {
    ...
    if [[ "$HTTPS2" == 200 ]] ; then
        ...
        echo "${RED}ROLEIT-API-CALL-ERROR====${NC}$HTTPS2"
        exit 0
    fi;
...
function fun_3() {
    ...
    if [[ "$HTTPS3" == 200 ]] ; then
        echo "${GRN}ROLEIT-API-CALL-DONE====${NC}$HTTPS3"
    else
        echo "${RED}ROLEIT-API-CALL-ERROR====${NC}$HTTPS3"
        exit 0
    fi;
...
function fun_4() {
    ...
    if [[ "$HTTPS4" == 200 ]] ; then
        echo "${GRN}ROLEIT-API-CALL-DONE====${NC}$HTTPS4"
    else
        echo "${RED}ROLEIT-API-CALL-ERROR====${NC}$HTTPS4"
        exit 0
    fi;
...
function fun_5() {
    ...
    if [[ "$HTTPS5" == 200 ]] ; then
        echo "${GRN}ROLEIT-API-CALL-DONE====${NC}$HTTPS5"
    else
        echo "${RED}ROLEIT-API-CALL-ERROR====${NC}$HTTPS5"
        exit 0
    fi;
